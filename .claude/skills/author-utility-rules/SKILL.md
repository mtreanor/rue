...
description: Multi-tier authoring process for rue utility rulesets — rules that adjust ephemeral numeric predicates (engagement-*, social-*, judge-*, claim-*) for use as action-selection utilities. Use when the user wants to author, extend, or review a ruleset like engagement-mode-rules.rue, socialize-rules.rue, judgement-rules.rue, or claim-judgement-rules.rue, or invokes /author-utility-rules.
---
...
Read `src/rue/src/AGENTS.md` and the target rules file before starting if
you haven't already this session.

## Session manifest

State for an authoring session lives in
`data/reception/authoring/<ruleset>.session.json`, sibling to the `.rue`
...
```json
{
  ...
  "rulesFile": "data/reception/engagement-mode-rules.rue",
...
2. **Which target numeric predicates** from that file's domain should rules
   ...
   actions/acts file as a starting menu, e.g. for `engagement-mode-rules.rue`
   that's `engagement-wait`, `engagement-approach`, `engagement-socialize`,
   `engagement-leave` (read `engagement-mode.rue` to confirm which utility
...
- `?OTHER`'s **history** — past occurrences they took part in, what they
  ...
  more as `judgement-acts.rue`/`judgement-rules.rue` get authored and
...
not self-perception**: `state.rue` asserts things like `-cool(drell)` and
...
dataset (the design doc and `state.rue` only ever assert them inside
...
2. **A tier-1 rule turns out to need a second premise for correctness** —
   ...
   mode-rules.rue "someone present" bug is the precedent: rules that
