# Where ML (RL / deep learning) could fit ruse

Ideation notes, organized by how cleanly each idea fits ruse's architecture — and
by how much it respects the "keep ruse a pure logic library" constraint. The
unifying theme: ruse's soundness lives in the symbolic core, so the safest ML lives
...
ruse already has `sensor` / `sensor-numeric` predicate types computed by
...
Attractive for ruse specifically: **provenance records are ready-made training
...
- **Inductive logic programming:** mine candidate `define` blocks or rules that
  ...
  propose rules that ruse then validates and runs symbolically.
...
into the evaluator — otherwise ruse stops being a deterministic logic library.
...
this is arguably agent cognition, scoped *out* of ruse per AGENTS.md. Application
layer.
