...
`tools/action-rule-set-tool` is a local web app for inspecting, searching, editing, and running RUE scenarios. Run it with:
...
1. **`RUE_CONFIG`** — an explicit override. Set it to a `project.config.json` path (or a directory containing one):
   ...
   ```
   RUE_CONFIG=/path/to/project.config.json npm run dev
   ```

   To set it once without retyping, create a gitignored `tools/action-rule-set-tool/.env` containing `RUE_CONFIG=/abs/path/to/project.config.json`.

2. **The host repo's config**, auto-discovered when RUE is a **git submodule**. Put a `project.config.json` at your repo root and the tool finds it — no configuration needed.

3. **RUE's own `project.config.json`** at the repo root (standalone development).

Scenario paths inside the config are resolved **relative to the config file's own directory**, so your config and data can live together anywhere — including a parent repo that vendors RUE as a submodule. The engine and syntax-highlighting grammar always load from the RUE submodule, so nothing about RUE needs editing.
...
Rules are rendered with DSL syntax highlighting, using the TextMate grammar from `extensions/vscode/rue.tmLanguage.json` (served by the backend), so tool colors always match the editor extension.
